...
================================================================================
【追記（最終確定・矛盾ゼロ版）】本追記が最優先（本文は維持、矛盾元記述は削除済み）
================================================================================

■ 1) explanation（解説）の扱い（最終確定）
- questions[].explanation は保持のみ。
- 結果TXTには 合格／不合格ともに一切出力しない。

■ 2) 設問別結果の表記粒度（最終確定）
- 合格時の設問別結果は「設問本文（body）＋正誤」を出力する。
- 正解選択肢（CID/テキスト）および解説（explanation）は出力しない。
- 不合格時は設問別結果を出力しない。
...
- answerPlain / answerPlainForDevOnly 等の平文参照経路は全面禁止。

■ 5) SVキー入力UIの運用制約（最終確定）
- 採点開始時にのみ入力可能な一時UI（モーダル等）で入力する。
- 採点完了後は即破棄し、再表示しない。
- 禁止：画面への再表示、ファイル保存、console出力、localStorage等への保存。

■ 6) builder.js の再編集方針（設計前提）
- 現時点の設計前提は「再編集なし」。
- UI は「新規作成 → ダウンロード」の一本道。
- 将来拡張（testpack.js 読み込み）は設計上の余白としてのみ考慮し、要件には含めない。


================================================================================
【整合性保証セクション（新設）】本仕様書の矛盾判定ルール（2026-01-06）
================================================================================

本セクションは「どの視点から確認しても矛盾が生じない」ことを目的とした
**仕様書内判定ルール**であり、本仕様書の一部として恒久的に適用される。

-------------------------------------------------------------------------------
A. 優先順位ルール（機械的判定）
-------------------------------------------------------------------------------
1. 本仕様書内で記述が衝突した場合、以下の優先順位で正とする。
   (1) 「【追記（最終確定・矛盾ゼロ版）】」
   (2) 本文中で後ろに記載された章
   (3) 箇条書きがある場合は、より具体的な記述
2. コード例・サンプル・進捗メモは説明補助であり、規則本文を上書きしない。
3. 本仕様書に明示されていない要求は「仕様外（新要求）」として扱う。

-------------------------------------------------------------------------------
B. 用語・識別子の正本定義（単一化）
-------------------------------------------------------------------------------
- testId : テスト識別の唯一の正本。表示・出力・管理の基準とする。
- testName : 表示用名称。識別には使用しない。
- attemptId : 受験単位の識別子。1受験につき1つ生成。
- fileName : 結果TXTの名称。10.1 の規則が唯一の正本。

-------------------------------------------------------------------------------
C. 禁止事項マトリクス（抜け道防止）
-------------------------------------------------------------------------------
以下の情報は、いかなる媒体・状態においても禁止とする。
...
禁止媒体：
- 画面表示（通常表示・結果表示・tooltip・title含む）
- ファイル出力（TXT / testpack.js）
- console 出力
...
制約：
- SVキー入力UIは「4. 採点中」にのみ存在してよい。
- 「5. 採点完了」到達時点でSVキーUIおよび値は破棄される。

-------------------------------------------------------------------------------
E. 受け入れテスト（Yes / No 判定）
-------------------------------------------------------------------------------
以下のいずれかに該当した場合、仕様違反と判定する。

- 合格時TXTに「設問本文＋正誤」以外が含まれる → 仕様違反
- 採点ロジックが平文正解を参照する → 仕様違反
- SVキーが画面・ファイル・console・Storage に残る → 仕様違反
- 結果TXTのファイル名が 10.1 に一致しない → 仕様違反
- 不合格時に設問別結果が出力される → 仕様違反

本セクションにより、本仕様書は
**視点差による解釈ブレを許容せず、自己完結的に矛盾判定が可能な状態**
であることを保証する。

-------------------------------------------------------------------------------
B-2. testpack 複数読込時の正本定義（格納先・識別・重複）
-------------------------------------------------------------------------------
- 複数の testpack は window.TESTPACKS（配列）に格納する。
- 各 testpack ファイルは読み込み時に window.TESTPACKS.push(TESTPACK_OBJECT) を実行する。
- 識別は meta.testId を唯一の正本とする。（B. 用語・識別子の正本定義に従う）
- 同一 meta.testId の重複登録は不可とする。
- 互換目的で window.TESTPACK が存在しても参照しない。（複数読込の正本は window.TESTPACKS）
