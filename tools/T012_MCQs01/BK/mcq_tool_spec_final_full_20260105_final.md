...
================================================================
【追記（最終確定・矛盾解消）】本追記が最優先（既存本文は保持）
================================================================

■ 解消1：explanation（解説）の扱い（最終確定）
- questions[].explanation は **保持のみ**。
- 結果TXTには **合格／不合格ともに一切出力しない**。
- これにより、本文 10.4 の
  「正しい選択肢や解説文は出力しない」規定を最優先とする。
- 過去の追記・履歴における
  「合格時のみ出力」と読める記述は **本追記により無効化**する。

■ 解消2：設問別結果の表記粒度（最終確定）
- 合格時の設問別結果は **設問本文（body）＋正誤** を出力する。
- 形式例は本文 10.4 のサンプルに従う。
...
- 識別子は **testId** を正とする。
- fileName 等への改名・二重管理は行わない。
- 結果TXTの命名規則は本文 10.1 を正とする。

■ 解消4：builder.js の再編集方針（設計前提）
- 現時点の設計前提は **再編集なし**。
- UI は「新規作成 → ダウンロード」の一本道。
- 将来拡張の余地（testpack.js 読み込み）は設計上の余白としてのみ考慮し、
  実装義務・要件には含めない。

■ 解消5：平文正解の完全禁止（再確認）
- 採点は **復号結果（CID配列）** のみを基準とする。
- 平文参照経路（answerPlain*）は **全面禁止**。
- builder.js / app.js 設計・実装は本規定を前提とする。
